...
{
    if (M[a.Lin][a.Col].val != CASUTA_IEPURE)
        return 0;
    if (M[b.Lin][b.Col].val != CASUTA_GOALA)
        return 0;
...
bool slide_vulpe_posibil(Casuta M[DIM_TABLA][DIM_TABLA],CoordTabla a, CoordTabla b, int directie)
{
    ...
    if (M[a.Lin][a.Col].val != CASUTA_VULPE_CAP_VERT)
        return 0;
    if (M[a.Lin][a.Col].val != CASUTA_VULPE_COADA_ORIZ)
        return 0;
    if (M[a.Lin][a.Col].val != CASUTA_VULPE_COADA_VERT)
        return 0;
    */
    if (M[b.Lin][b.Col].val != CASUTA_GOALA)
        return 0;
...
{
    //vulpe oriz - V (capul se duce acolo unde pun clickul,coada urmeaza la aceeasi lin, col-1)
    //           - E (coada se duce mai intai, capul urmeaza la aceeasi lin, col+1)
    //vulpe vert - N ( se duce capul, iar corpul la lin-1, aceeasi col)
    //           - S (se duce coada unde dau click, capul vine la lin+1, aceeasi col )
